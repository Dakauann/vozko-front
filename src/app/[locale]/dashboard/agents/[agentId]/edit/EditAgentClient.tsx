...
function EditChooserCard({
  ...
  return (
    <motion.button
      ...
      whileHover={{ y: -4 }}
      whileTap={{ scale: 0.98 }}
      className="group relative flex h-full flex-col rounded-3xl border border-border bg-card p-7 text-left shadow-sm transition-all hover:border-primary/40 hover:shadow-lg max-w-xl"
    >
      {recommended && recommendedLabel ? (
        <span className="absolute right-5 top-5 inline-flex items-center gap-1 rounded-full bg-gradient-to-br from-primary to-[hsl(var(--primary-hover))] px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-primary-foreground shadow">
          <Sparkle className="h-3 w-3" weight="fill" />
          {recommendedLabel}
        </span>
      ) : null}
      <div
        className={`flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br ${iconGradient} text-white shadow-lg`}
      >
      ...
      <div className="mt-auto flex items-center justify-between pt-6">
        ...
        <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-primary to-[hsl(var(--primary-hover))] text-primary-foreground shadow-md transition-transform group-hover:translate-x-1">
